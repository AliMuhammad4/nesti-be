...
test('late segments never downgrade minutes_status from ready to pending', async () => {
  // Reset the shared upsert-state so this write is treated as a fresh upsert.
  stored = null;
  callUpdates = [];
  const persisted = await persistFinalTranscriptSegment({
    callId: '64b000000000000000000004',
    segmentId: 'late-segment-1',
    participant: { identity: 'user-2', name: 'Casey' },
    publication: { sid: 'track-2' },
    alternative: {
      text: 'A late straggler line.',
      language: 'en',
      startTime: 42.0,
      endTime: 42.5,
      confidence: 0.95,
  ...
  const normalization = callUpdates.find(
    (entry) => entry.update?.$set?.minutes_status === 'pending',
  );
  assert.ok(
    normalization,
    'expected a normalization update that would set minutes_status to pending',
  );
  // The critical guard: this update must skip calls whose minutes are already ready.
  assert.deepEqual(normalization.filter.minutes_status, { $ne: 'ready' });
  assert.deepEqual(normalization.filter.status.$in, ['ended', 'expired']);
  assert.deepEqual(normalization.filter.transcription_status.$in, ['completed', 'disabled']);
});

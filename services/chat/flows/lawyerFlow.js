...
} from '../scoring/lawyerScoring.js';
import { mergeSignals } from '../scoring/common.js';
...
export const lawyerFlow = {
  ...
  createNewLead: async (params) => {
    ...
    return createLawyerLeadRecords({
      conversation,
  ...
  getLeadClassification: (finalGrade) => `${finalGrade.charAt(0).toUpperCase() + finalGrade.slice(1)} Lawyer Lead`,
  ...
  getMergedAiDetails: (parsedAiDetails, derivedQual) => ({
    ...
    preferred_contact_method: parsedAiDetails?.preferred_contact_method || derivedQual.preferred_contact_method || '',
  ...
  getLeadProfileUpdate: (parsedAiDetails, derivedQual, formContact) => ({
    transaction_stage:     parsedAiDetails?.transaction_stage     || derivedQual.transaction_stage || formContact?.transaction_stage,
    closing_timeline:      parsedAiDetails?.closing_timeline      || derivedQual.closing_timeline || formContact?.closing_timeline,
    transaction_type:     parsedAiDetails?.transaction_type     || derivedQual.transaction_type || formContact?.transaction_type,
    property_value:       parsedAiDetails?.property_value       || derivedQual.property_value || formContact?.property_value,
    mortgage_status:      parsedAiDetails?.mortgage_status      || derivedQual.mortgage_status || formContact?.mortgage_status,
    realtor_involved:     parsedAiDetails?.realtor_involved     || derivedQual.realtor_involved || formContact?.realtor_involved,
    first_time_buyer:     parsedAiDetails?.first_time_buyer     || derivedQual.first_time_buyer || formContact?.first_time_buyer,
    legal_services_needed: parsedAiDetails?.legal_services_needed || derivedQual.legal_services_needed || formContact?.legal_services_needed,
    preferred_contact_method: parsedAiDetails?.preferred_contact_method || derivedQual.preferred_contact_method || formContact?.preferred_contact_method,
    best_time_to_contact: parsedAiDetails?.best_time_to_contact || derivedQual.best_time_to_contact || formContact?.best_time_to_contact,
    property_address:    parsedAiDetails?.property_address    || formContact?.address,
    location:             parsedAiDetails?.location            || parsedAiDetails?.property_address || formContact?.location,
    budget:               parsedAiDetails?.budget              || parsedAiDetails?.property_value || formContact?.budget,
  }),
};
